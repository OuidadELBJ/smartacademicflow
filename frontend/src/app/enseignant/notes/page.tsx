...
interface NoteData {
  ...
  valeur: number | null; isBlockedByArticle39: boolean; typeEvaluation: string;
}
...
export default function SaisieNotesPage() {
    ...
    const fetchElements = async () => {
      try {
        ...
        if (res.data.length > 0) {
          setSelectedElement(res.data[0].id);
        }
  ...
  // Load etudiants + notes when element or tab changes
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        // Get filiere prefix for filtering
        const filierePrefix = el.filiereCode.toLowerCase().replace(/&/g, "").replace(/\s/g, "");

        // Fetch etudiants AND notes in parallel
        const [etuRes, notesRes] = await Promise.all([
          api.get(`/enseignant/etudiants?filiere=${encodeURIComponent(filierePrefix)}`),
          api.get(`/enseignant/notes/element/${selectedElement}?type=${activeTab}`),
        ]);

        const etudiants: Etudiant[] = etuRes.data;
        const existingNotes: NoteData[] = notesRes.data;
        const notesMap = new Map(existingNotes.map((n: NoteData) => [n.etudiantId, n]));

        // Merge: show all students, fill with existing notes
        ...
        console.error("Erreur chargement notes/etudiants:", err);
        setNotes([]);
      } finally {
    ...
    <DashboardLayout>
        ...
        <div className="flex gap-1 mb-4 p-1 bg-slate-100 rounded-xl w-fit">
          {availableTabs.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={cn("px-4 py-2 rounded-lg text-xs font-medium transition-all", activeTab === tab ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700")}>
              {tab === "EXAM" ? "Examen" : tab}
            </button>
          ))}
        </div>
      ...
      {activeTab === "EXAM" && selectedElement && (
        <div className="mb-4 p-3 rounded-2xl bg-red-50 border border-red-100 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-red-500 text-[11px] mt-0.5">Absence injustifiee a l'examen = note forcee a 0/20 et verrouillee. Ne concerne pas TD/TP/Projet.</p>
          </div>
        </div>
      )}
      ...
      {selectedElement && (
        <div className="card p-0 overflow-hidden">
          ...
          {loadingNotes ? (
            <div className="py-12 text-center text-slate-400 text-sm animate-pulse">Chargement des notes...</div>
          ) : (
                    ...
                    <tr key={note.etudiantId} className={cn("border-b border-slate-100/80 transition-colors", note.isBlockedByArticle39 ? "row-blocked-article39" : "hover:bg-slate-50/50")}>
                      ...
                      <td className="px-6 py-2.5">
                        <div className="flex items-center gap-2">
                          {note.isBlockedByArticle39 && <div className="w-5 h-5 rounded bg-red-50 flex items-center justify-center"><Lock size={10} className="text-red-400" strokeWidth={2} /></div>}
                          <div>
                            <p className={cn("text-sm font-medium", note.isBlockedByArticle39 ? "text-slate-400" : "text-slate-800")}>{note.etudiantNom} {note.etudiantPrenom}</p>
