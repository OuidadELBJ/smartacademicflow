...
export default function ValidationPVPage() {
  ...
  const handleValidation = async () => {
    ...
    try {
      ...
      setModules((prev) =>
        prev.map((m) => ({ ...m, statut: "CLOTURE" as const }))
      );
      setConfirming(false);
    } catch (err) {
      console.error(err);
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-slate-900 text-xl font-bold">
              Validation du PV Semestriel
            </h1>
            <p className="text-slate-500 text-xs">
              Filiere : Genie Logiciel (GL) | Semestre : S1 2024-2025
            </p>
          </div>
        </div>
        ...
        <div className="mb-6 p-4 rounded-2xl bg-red-50 border border-red-100 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-red-500 text-xs mt-0.5">
              La validation du PV cloture definitivement tous les modules. Aucune
              modification des notes ne sera possible apres cette operation.
            </p>
          </div>
        </div>
          ...
          {modules.map((module) => (
            <div
              ...
              className={cn(
                ...
                module.statut === "CLOTURE" && "opacity-60"
              )}
            >
              <div className={cn(
                "w-10 h-10 rounded-xl flex items-center justify-center",
                module.statut === "CLOTURE" ? "bg-slate-100" : "bg-blue-50"
              )}>
                {module.statut === "CLOTURE" ? (
                  <Lock size={18} className="text-slate-400" strokeWidth={1.5} />
                ) : (
                  <BookOpen size={18} className="text-blue-600" strokeWidth={1.5} />
                )}
              </div>
              ...
              <div className="w-32">
                ...
                <div className="w-full h-1.5 bg-slate-100 rounded-full">
                  <div
                    className={cn(
                      "h-full rounded-full",
                      module.progression === 100 ? "bg-emerald-500" : "bg-blue-500"
                    )}
          ...
          {confirming ? (
            <div>
              <p className="text-red-600 text-sm font-medium mb-3">
                Etes-vous certain de vouloir valider le PV ? Cette action est irreversible.
              </p>
              <div className="flex gap-3 justify-center">
                <button onClick={handleValidation} className="btn-danger">
            ...
            <button
              ...
              disabled={!allReady}
              className={cn(
                "btn-primary justify-center px-8",
                !allReady && "opacity-50 cursor-not-allowed"
              )}
            >
              <CheckCircle size={16} strokeWidth={1.5} />
              Valider le PV Semestriel
            </button>
          )}
        </div>
      </div>
