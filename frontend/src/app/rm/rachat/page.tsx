    ...
    <DashboardLayout>
      <div className="flex items-center gap-3 mb-8">
        <div className="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center">
          <ArrowUpCircle size={20} className="text-blue-600" strokeWidth={1.5} />
        </div>
        <div>
          <h1 className="text-slate-900 text-xl font-bold">Rachat de Notes</h1>
          <p className="text-slate-500 text-xs">
            Modifier une note avec motif obligatoire (Art. 35-37)
          </p>
        </div>
      </div>
      ...
      <div className="card mb-6 bg-blue-50/50 border-blue-100">
        <div className="flex items-start gap-3">
          <AlertCircle size={16} className="text-blue-600 mt-0.5 shrink-0" strokeWidth={1.5} />
            ...
            {notes.map((note) => (
              <div
                ...
                className={cn(
                  ...
                  note.isArticle39
                    ? "row-blocked-article39 cursor-not-allowed"
                    : selectedNote === note.id
                    ? "bg-blue-50/50"
                    : "hover:bg-slate-50/50"
                )}
                onClick={() => !note.isArticle39 && setSelectedNote(note.id)}
              >
                {note.isArticle39 && (
                  <Lock size={14} className="text-red-400 shrink-0" strokeWidth={2} />
                )}
                <div className="flex-1">
                  <p className={cn(
                    "text-sm font-medium",
                    note.isArticle39 ? "text-slate-400" : "text-slate-800"
                  )}>
                    {note.etudiant}
                  </p>
                ...
                <span className={cn(
                  "text-lg font-bold",
                  note.isArticle39 ? "text-red-300" : "text-amber-600"
                )}>
                  {note.note}/20
                </span>
              </div>
        ...
        <div className="card">
          ...
          {selectedNote ? (
            <div className="space-y-4">
              ...
              <div>
                <label className="text-slate-600 text-xs font-medium mb-1 block">
                  Motif du rachat (obligatoire)
                </label>
                <textarea
                  value={motif}
                  onChange={(e) => setMotif(e.target.value)}
                  className="input-field min-h-[100px] resize-none"
                  placeholder="Justification detaillee du rachat..."
                />
              </div>
              <button
                disabled={!motif.trim() || !nouvelleNote}
                className={cn(
            ...
            <div className="text-center py-8">
              ...
              <p className="text-slate-400 text-xs">
                Selectionnez une note dans la liste pour effectuer un rachat
              </p>
            </div>
          )}
        </div>
